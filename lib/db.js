...
/** Append pooler/SSL flags without parsing the URI (avoids mangling passwords). */
function normalizeConnectionString(raw) {
  if (!raw) return raw
  let url = String(raw).trim()

  if (/[?&]sslmode=/.test(url)) {
    url = url.replace(/([?&])sslmode=[^&]*/g, '$1sslmode=no-verify')
  } else {
...
export async function pingDatabase() {
  ...
  const connectionMeta = summarizeConnectionString(raw)
  ...
  try {
    ...
    return { ok: true, ...result.rows[0], connection: connectionMeta }
  } catch (error) {
...
function summarizeConnectionString(raw) {
  if (!raw) return null
  try {
    ...
    if (!match) {
      return { parseOk: false, hint: 'DATABASE_URL format could not be parsed' }
    }
    ...
    return {
      ...
      user,
      host,
      port: port || (host.includes('pooler') ? '6543' : '5432'),
      database: database || 'postgres',
      passwordLength: password ? password.length : 0,
      passwordLooksEncoded: Boolean(password && /%[0-9A-Fa-f]{2}/.test(password)),
      isPoolerHost: host.includes('pooler.supabase.com'),
      userLooksLikePooler: user.includes('.'),
      hint:
        host.includes('pooler.supabase.com') && !user.includes('.')
          ? 'Pooler host usually needs username postgres.PROJECT_REF, not just postgres'
          : null,
    }
  } catch {
